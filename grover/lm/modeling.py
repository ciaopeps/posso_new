...
def _top_p_sample(logits, ignore_ids=None, num_samples=1, p=0.9):
    ...
    with tf.variable_scope('top_p_sample'):
        ...
        # OPTION A - encoder in the sorted space, then unsort.
        ...
        # OPTION B - unsort first - Indices need to go back to 0 -> N-1 -- then encoder
        ...
        # encoder = tf.random.categorical(logits=logits_to_use, num_samples=num_samples, dtype=tf.int32)
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if use_tpu:
                ...
                predictions = tf.reshape(
                    _top_p_sample(model.logits_flat, num_samples=1, p=0.99)['encoder'],
                    get_shape_list(model.target_ids),
                )
...
def sample_step(tokens, ignore_ids, news_config, batch_size=1, p_for_topp=0.95, cache=None, do_topk=False):
    ...
    new_tokens = tf.squeeze(sample_info['encoder'], 1)
    new_probs = tf.squeeze(tf.batch_gather(sample_info['probs'], sample_info['encoder']), 1)
    return {
...
def sample(news_config: GroverConfig, initial_context, eos_token, ignore_ids=None, p_for_topp=0.95,
           do_topk=False):
    """
    V1 version of: encoder outputs from a model, and do it all at once
